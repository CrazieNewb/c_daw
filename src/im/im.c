...
void imlog(uint8_t r, uint8_t g, uint8_t b, const char* prefix, const char* format, va_list args) {
	printf("\033[1m\033[38;2;%d;%d;%dm", r, g, b);
	printf("[%s] ", prefix);
	printf("\033[0m");
...
imlogcallback_t* imlogcallback = &imlog;
...
void iminfo(const char* format, ...) {
	...
	(imlogcallback)(230, 240, 255, "INFO", format, args);
	va_end (args);
}
...
void imwarn(const char* format, ...) {
	va_list args;
	va_start (args, format);
	(imlogcallback)(255, 180, 80, "WARN", format, args);
	va_end (args);
}
...
void imerror(const char* format, ...) {
	va_list args;
	va_start (args, format);
	(imlogcallback)(255, 80, 80, "ERROR", format, args);
	va_end (args);
}
